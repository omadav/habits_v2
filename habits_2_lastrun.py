...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'C:\\Users\\omada\\Dropbox\\Projects\\Caltech\\Humans\\Habits\\Caltech 2018-2 (Anastasia)\\Task\\habits_2.psyexp',
...
instr_test_txt = visual.TextStim(win=win, name='instr_test_txt',
    text='In this experiment, you will be using your joystick to earn coins that will be cashed for real money at the end of the experiment.\n\nDuring each trial, you will be allowed to perform one of four actions with the joystick: move left, move right, move up, move down.\n\nYou will know which action is the correct one by a stimulus that will appear on the screen.\n\nThe following screen will show the stimuli and the correct action you need to make when you see it.\n\n(Press any key to continue)',
    font='Comic Sans',
...
instr_stim1_txt = visual.TextStim(win=win, name='instr_stim1_txt',
    text='Below is one of the stimuli you will see.\n\nAs you can appreciate, it is a cube that has a button on one of its sides.\n\nIn this case, the button is on top of the cube, so the correct response is to push the button by moving the joystick down, as shown in the picture to the right of the cube.\n\n\n\n\n\n\n\n\n\n\n(Press any key to continue)',
    font='Comic Sans',
    pos=(0, 0), height=0.06, wrapWidth=None, ori=0, 
    color='black', colorSpace='rgb', opacity=1,
    depth=0.0);
...
instr_stim2_txt = visual.TextStim(win=win, name='instr_stim2_txt',
    text=u'Here is another one of the stimuli you will see.\n\nIn this case, the button is on the left of the cube, so the correct response is to push the button by moving the joystick to the right, as shown in the picture to the right of the cube.\n\n\n\n\n\n\n\n\n\n\n(Press any key to continue)',
    font=u'Comic Sans',
    pos=(0, 0), height=0.06, wrapWidth=None, ori=0, 
...
instr_stim3_txt = visual.TextStim(win=win, name='instr_stim3_txt',
    text=u'Here is another one of the stimuli you will see.\n\nIn this case, the button is at the bottom of the cube, so the correct response is to push the button by moving the joystick up, as shown in the picture to the right of the cube.\n\n\n\n\n\n\n\n\n\n\n(Press any key to continue)',
    font=u'Comic Sans',
    pos=(0, 0), height=0.06, wrapWidth=None, ori=0, 
    color=u'black', colorSpace='rgb', opacity=1,
    depth=0.0);
...
instr_stim4_txt = visual.TextStim(win=win, name='instr_stim4_txt',
    text=u'And here is another one of the stimuli you will see.\n\nIn this case, the button is on the right of the cube, so the correct response is to push the button by moving the joystick to the left, as shown in the picture to the right of the cube.\n\n\n\n\n\n\n\n\n\n\n(Press any key to continue)',
    font=u'Comic Sans',
    pos=(0, 0), height=0.06, wrapWidth=None, ori=0, 
    color=u'black', colorSpace='rgb', opacity=1,
    depth=0.0);
...
instr2_3_txt = visual.TextStim(win=win, name='instr2_3_txt',
    text=u'Every time you perform the action that corresponds to the stimulus shown on the screen, a triangle will appear at the center of the screen signalling that a response has been made. \n\nEvery time you receive a reward for performing the action that corresponds to the stimulus shown on the screen you will receive either a gold or silver coin. These will appear at the top of the screen.\n\nTwo of the actions will give you gold coins when you are rewarded; the other two will give you silver coins. \n\nEach of the coins you win during the experiment will be deposited in one of two piggy banks, one for gold coins and the other for silver coins.\n\nPlease note that given the nature of the task, it is to your advantage to understand what is the best way to perform the actions. All actions are similar in that there is one way to earn the most rewards that applies to all of them. But you need to figure this out yourself!\n\nGood luck!\n\n(Press any key to continue)',
    font=u'Comic Sans',
    pos=(0, 0), height=0.06, wrapWidth=None, ori=0, 
    color=u'black', colorSpace='rgb', opacity=1,
    depth=0.0);
...
def resp_made():
    ...
    # show the response figure and the stim_pressed img for 30 frames
#    for nframes in range(30):
...
stim_choice1 = visual.ImageStim(
    ...
    flipHoriz=False, flipVert=False,
    texRes=128, interpolate=True, depth=-2.0)
stim_pressed_choice1 = visual.ImageStim(
    win=win, name='stim_pressed_choice1',
    image='sin', mask=None,
    ori=0, pos=[0,0], size=(0.3, 0.3),
...
stim_pressed_choice2 = visual.ImageStim(
    win=win, name='stim_pressed_choice2',
    image='sin', mask=None,
    ori=0, pos=[0,0], size=(0.3, 0.3),
    color=[1,1,1], colorSpace='rgb', opacity=0,
    flipHoriz=False, flipVert=False,
...
# keep track of which components have finished
instr_stim1Components = [instr_stim1_txt, key_resp_6, resp_down_img, joy_down_img]
...
# keep track of which components have finished
instr_stim2Components = [instr_stim2_txt, key_resp_7, resp_right_img, joy_right_img]
...
trials = data.TrialHandler(nReps=2, method='random', 
    ...
    trialList=data.importConditions(u'conditions.xlsx', selection=u'0:4'),
...
for thisTrial in trials:
    ...
    while continueRoutine:
        ...
        if joy_posx_array not in [[1, 1], [-1,-1]] and joy_posy_array not in [[1, 1], [-1,-1]]: # pos_array not in [["left","left"], ["right","right"], ["up", "up"],["down","down"]]:
            ...
            if joy_y == -1:
                ...
                if key_resp_2.keys in [corrResp]:
                    ...
                    for nframes in range(5):
                        ...
                        credits_title_txt.draw()
                        num_credits_txt.draw()
                        stim.setImage(stimulus_pressed)
                        stim.draw()
                        win.flip()
                resp_made() 
            elif joy_y == 1:
            ...
            stim.setImage(stimulus) # set the not pressed stim again
...
test = data.TrialHandler(nReps=1, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions(u'conditions.xlsx', selection=u'4:6'),
    seed=None, name='test')
...
for thisTest in test:
    ...
    stim_pressed_choice1.setPos(position1)
    stim_pressed_choice1.setImage(stimulus_pressed_choice1)
    stim_choice_2.setPos(position2)
    stim_choice_2.setImage(stimulus_choice2)
    stim_pressed_choice2.setPos(position2)
    stim_pressed_choice2.setImage(stimulus_pressed_choice2)
    ...
    # keep track of which components have finished
    choice_testComponents = [circle_resp_choice, key_resp_4, stim_choice1, stim_pressed_choice1, stim_choice_2, stim_pressed_choice2, curtain_img]
    ...
    while continueRoutine:
        ...
        # *stim_pressed_choice1* updates
        if t >= 0.0 and stim_pressed_choice1.status == NOT_STARTED:
            # keep track of start time/frame for later
            stim_pressed_choice1.tStart = t
            stim_pressed_choice1.frameNStart = frameN  # exact frame index
            stim_pressed_choice1.setAutoDraw(True)
        frameRemains = 0.0 + duration- win.monitorFramePeriod * 0.75  # most of one frame period left
        if stim_pressed_choice1.status == STARTED and t >= frameRemains:
            stim_pressed_choice1.setAutoDraw(False)
        ...
        # *stim_pressed_choice2* updates
        if t >= 0.0 and stim_pressed_choice2.status == NOT_STARTED:
            # keep track of start time/frame for later
            stim_pressed_choice2.tStart = t
            stim_pressed_choice2.frameNStart = frameN  # exact frame index
            stim_pressed_choice2.setAutoDraw(True)
        frameRemains = 0.0 + duration- win.monitorFramePeriod * 0.75  # most of one frame period left
        if stim_pressed_choice2.status == STARTED and t >= frameRemains:
            stim_pressed_choice2.setAutoDraw(False)
